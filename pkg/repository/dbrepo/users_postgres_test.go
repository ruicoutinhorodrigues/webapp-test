...
import (
	...
	"local/rui.rodrigues/webapp-test/pkg/repository"
...
func Test_PostgresDBRepo_InsertUser(t *testing.T) {
	...
	if id != 1 {
		t.Errorf("insert user returned the wrong id; expected 1 but got %d", id)
	}
}
...
func Test_PostgresDBRepo_AllUsers(t *testing.T) {
	...
	if len(users) != 1 {
		t.Errorf("all users reports the wrong size; expected 1 but got %d", len(users))
	}

	userID := users[0].ID

	if userID != 1 {
		t.Errorf("got the wrong user ID; expected 1 but got %d", userID)
	}
	...
	userID, _ = testRepo.InsertUser(testUser2)

	users, _ = testRepo.AllUsers()

	if len(users) != 2 {
		t.Errorf("all users reports the wrong size; expected 2 but got %d", len(users))
	}

	if userID != 2 {
		t.Errorf("got the wrong user ID; expected 2 but got %d", userID)
	}
}
...
func Test_PostgresDBRepo_GetUserByEmail(t *testing.T) {
	...
	if user.ID != 2 {
		t.Errorf("wrong ID returned by GetUserEmail; expected 2 but got %d", user.ID)
	}
}
...
func Test_PostgresDBRepo_UpdateUser(t *testing.T) {
	...
	if err != nil {
		t.Errorf("error updating user with ID %d: %s", 2, err)
	}

	user, _ = testRepo.GetUser(2)
	if user.FirstName != "Jane" || user.Email != "jane@example.com" {
		t.Errorf("expected updated record to have first name Jane and email jane@exampla.com but got %s and %s, respectively", user.FirstName, user.Email)
	}
}
...
func Test_PostgresDBRepo_ResetPassword(t *testing.T) {
	...
	if err != nil {
		t.Error("error resetting the user password", err)
	}
...
func Test_PostgresDBRepo_InsertUserImage(t *testing.T) {
	...
	if newID != 1 {
		t.Errorf("got the wrong ID for the image; should bve 1 but got %d", newID)
	}
